({
	...
	doInit : function( component, event, helper ) {
       ...
       if(!$A.util.isEmpty(component.get('v.label'))){
        component.find("inputLookup").set("v.value",component.get('v.label'));
       }
	...
	selectItem : function( component, event, helper ) {
        ...
        console.log('Enter in selectItem method');
        if(!$A.util.isEmpty(event.currentTarget.id)) {
    		var recordsList = component.get('v.recordsList');
            var index = recordsList.findIndex(x => x.value === event.currentTarget.id);
            console.log('index is ',index,event.currentTarget.id);
            if(index != -1) {
                var selectedRecord = recordsList[index];
            }
            console.log('selectedrecord is ',selectedRecord);
            component.set('v.selectedRecord',selectedRecord);
            component.set('v.value',selectedRecord.value);
            component.set('v.label',selectedRecord.label);
            $A.util.removeClass(component.find('resultsDiv'),'slds-is-open');
        }
